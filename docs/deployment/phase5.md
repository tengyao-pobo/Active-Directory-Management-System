...
未設定、同步失敗、過期/不可用、拒絕存取與物件不存在分別顯示，不使用示範資料補值。保護判定未知仍顯示 Unknown。總覽 Connector readiness 也改讀 status API。狀態目前在進入頁面或按「重新整理目錄」時讀取，沒有背景輪詢。
...
新增 `/#/search`，可一次提交名稱或帳號至目前環境的四種目錄類型。空白搜尋不發送請求；查詢限制為 128 字元，使用既有 URL 編碼與 API scope 過濾。各類型保留獨立分頁、明細、來源時間與錯誤狀態；不提供可能誤導的全域總數，也不宣稱四種結果來自同一交易快照。單一類型失敗不會隱藏其他類型結果。

新查詢、清除或切換環境會卸載先前結果並取消請求。搜尋文字不寫入 URL、瀏覽器儲存或伺服器偏好。各類型可重新整理，重新整理會重設該類分頁與明細；清單失敗時不再顯示 Snapshot ready。使用者可從側邊欄開啟跨類型搜尋，頂部 Search pages 仍是頁面導覽篩選。

此增量未更動後端授權、資料庫或 AD adapter。新增瀏覽器驗證涵蓋四種查詢、空白輸入、字面編碼、清除、新查詢重設分頁/明細、環境切換、過晚回應、單類型失敗及未配置狀態恢復。詳細 CI 結果見對應 PR。
...
跨類型搜尋增量本機驗證：build/lint 通過，13 項單元測試與 44 項桌面/手機瀏覽器測試全數通過；已檢視搜尋畫面。測試仍使用合成 API fixture，不代表真實網域驗收。
