...
隔離資料庫若使用不同的用途角色，可設定 `CONSOLE_TEST_PLAN_LOCK_OWNER`、`CONSOLE_TEST_EXECUTION_DEFINER` 與 `CONSOLE_TEST_QUEUE_DEFINER`；預設分別為 `console_enrollment_plan_locker`、`console_execution_definer`、`console_execution_queue_definer`。執行用途的 psql 測試可用 `CONSOLE_TEST_PSQL` 指定 executable。平台 execution 與 Agent private 的正式部署必須分庫；Agent 測試 fixtures 可透過各自的 `AGENT_*_TEST_DB` 指向另外的 loopback `console_test` 或 `console_ci`，仍保持專案循序執行。
