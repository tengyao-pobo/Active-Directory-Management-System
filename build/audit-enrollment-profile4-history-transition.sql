...
DECLARE
    ...
    expected_audit_hash constant text := 'a05146573ba7d57bcbe9c027eddf37d730bd20fb827a91ac8cdb36c77e0a1c5b';
    ...
    expected_runtime_audit_hash constant text := '4a17d8703cafed7884df3b5621032ce9e4326f4973b845360c6cc6fccab1f4ee';
...
SELECT (
WITH owner_role AS (
 SELECT oid FROM pg_catalog.pg_roles WHERE rolname=(SELECT rolname FROM pg_catalog.pg_roles WHERE oid=owner_oid)
), guard AS (
...
SELECT COALESCE((SELECT
 ...
 WHERE p.oid=pg_catalog.to_regprocedure('enrollment_execution.guard_profile4_publication()')
 AND pg_catalog.encode(pg_catalog.sha256(pg_catalog.convert_to(pg_catalog.replace(p.prosrc,E'\r\n',E'\n'),'UTF8')),'hex')='1788f276fbb79bea8210ec3e0bfca1cdda1fd3f8b84f969ff627b9e15e244718') AS is_valid
) IS NOT TRUE THEN
        RAISE EXCEPTION USING ERRCODE='55000',MESSAGE='History publication guard contract is invalid.';
    END IF;
