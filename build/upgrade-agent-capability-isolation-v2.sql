...
WITH audit AS(
 ...
 lifecycle AS(
 SELECT CASE
  WHEN audit.body_hash='bde901e858a456dd6d9fa273cc54c354' AND
  ...
  AND
   pg_catalog.to_regprocedure('agent_private.read_initial_enrollment_grant_status(uuid,uuid,uuid,uuid,uuid,timestamptz,bytea,bytea,timestamptz,timestamptz)') IS NOT NULL AND
   pg_catalog.to_regprocedure('agent_private.read_initial_enrollment_grant_status(uuid,uuid,uuid,uuid,uuid,timestamptz,bytea,bytea,timestamptz,timestamptz,smallint,timestamptz)') IS NOT NULL AND
   (SELECT pg_catalog.count(*)=8 AND pg_catalog.bool_and(pg_catalog.md5(pg_catalog.btrim(function.prosrc,E' \t\r\n'))=expected.body_hash) FROM (VALUES
      (pg_catalog.to_regprocedure('agent_private.issue_initial_enrollment_grant(uuid,uuid,uuid,uuid,timestamptz,bytea,bytea)'),'1ebfa7beba1c293223ece3f8396fdce6'),
      (pg_catalog.to_regprocedure('agent_private.issue_initial_enrollment_grant(uuid,uuid,uuid,uuid,timestamptz,timestamptz,bytea,bytea)'),'1bf8dc3b09be84738a26248171cc7388'),
      (pg_catalog.to_regprocedure('agent_private.read_initial_enrollment_grant(uuid,uuid,uuid,uuid,uuid,timestamptz,bytea,bytea,timestamptz,timestamptz,smallint,timestamptz)'),'79947e1d1e6ce34ef418c9f12eda2b1f'),
      (pg_catalog.to_regprocedure('agent_private.read_initial_enrollment_grant(uuid,uuid,uuid,uuid,uuid,timestamptz,bytea,bytea,timestamptz,timestamptz)'),'083af3e33f15d59b9353ef99d68cf0ef'),
      (pg_catalog.to_regprocedure('agent_private.revoke_initial_enrollment_grant(uuid,uuid,uuid,uuid,uuid,uuid,timestamptz,bytea,bytea,timestamptz,timestamptz,smallint,timestamptz,bytea)'),'4302939045540eb758ddcdea030ab20d'),
      (pg_catalog.to_regprocedure('agent_private.revoke_initial_enrollment_grant(uuid,uuid,uuid,uuid,uuid,uuid,timestamptz,bytea,bytea,timestamptz,timestamptz,bytea)'),'d08e43e2b6bed6ba8499b73d18a0bfa4'),
      (pg_catalog.to_regprocedure('agent_private.read_initial_enrollment_grant_status(uuid,uuid,uuid,uuid,uuid,timestamptz,bytea,bytea,timestamptz,timestamptz,smallint,timestamptz)'),'9550dae066961bde6e7ca1d10cb10324'),
      (pg_catalog.to_regprocedure('agent_private.read_initial_enrollment_grant_status(uuid,uuid,uuid,uuid,uuid,timestamptz,bytea,bytea,timestamptz,timestamptz)'),'2eeb2c460b0d1ef45f7f3981697f566e')) expected(signature,body_hash) JOIN pg_catalog.pg_proc function ON function.oid=expected.signature::oid)
  THEN 4::smallint
...
WHERE lifecycle.profile_version IS NOT NULL AND lifecycle.owner_name=:'agent_platform_grant_definer_role' AND
 ...
 (lifecycle.profile_version=1 OR (
  ...
  (SELECT pg_catalog.count(*)=CASE lifecycle.profile_version WHEN 2 THEN 4 WHEN 3 THEN 7 ELSE 9 END AND pg_catalog.bool_and(function.proowner=lifecycle.proowner AND function.prosecdef AND function.provolatile='v' AND function.proparallel='u' AND function.proconfig=ARRAY['search_path=pg_catalog, agent_private, pg_temp']::text[]) FROM pg_catalog.pg_proc function WHERE function.oid=ANY(ARRAY[
   ...
   pg_catalog.to_regprocedure('agent_private.read_initial_enrollment_grant_status(uuid,uuid,uuid,uuid,uuid,timestamptz,bytea,bytea,timestamptz,timestamptz)'),
   pg_catalog.to_regprocedure('agent_private.read_initial_enrollment_grant_status(uuid,uuid,uuid,uuid,uuid,timestamptz,bytea,bytea,timestamptz,timestamptz,smallint,timestamptz)'),
  ...
  (SELECT pg_catalog.count(*)=1 FROM pg_catalog.pg_constraint constraint_info WHERE constraint_info.conrelid='agent_private.platform_grant_database_bindings'::pg_catalog.regclass AND constraint_info.contype='c' AND ((lifecycle.profile_version=4 AND pg_catalog.pg_get_constraintdef(constraint_info.oid)='CHECK ((purpose = ANY (ARRAY[''IssueInitialGrant''::text, ''RevokeInitialGrant''::text, ''ReadInitialGrantStatus''::text])))') OR (lifecycle.profile_version<>4 AND pg_catalog.pg_get_constraintdef(constraint_info.oid) LIKE '%IssueInitialGrant%' AND pg_catalog.pg_get_constraintdef(constraint_info.oid) LIKE '%RevokeInitialGrant%'))) AND
  ...
  (SELECT pg_catalog.count(*) FROM pg_catalog.pg_proc function,LATERAL pg_catalog.aclexplode(COALESCE(function.proacl,pg_catalog.acldefault('f',function.proowner))) acl WHERE function.oid=ANY(ARRAY[
   ...
   CASE lifecycle.profile_version WHEN 2 THEN 4 WHEN 3 THEN 7 ELSE 9 END+(SELECT COALESCE(pg_catalog.sum(CASE purpose WHEN 'IssueInitialGrant' THEN CASE lifecycle.profile_version WHEN 2 THEN 2 ELSE 3 END WHEN 'RevokeInitialGrant' THEN CASE lifecycle.profile_version WHEN 2 THEN 3 ELSE 5 END WHEN 'ReadInitialGrantStatus' THEN CASE lifecycle.profile_version WHEN 4 THEN 3 ELSE 1000 END ELSE 1000 END),0) FROM agent_private.platform_grant_database_bindings)
  )));
...
BEGIN
 SELECT source.audit_definition INTO v_definition FROM pg_temp.platform_grant_lifecycle_source source WHERE source.profile_version IN(2,3,4);
...
SELECT 1 WHERE
 ...
 (SELECT CASE source.profile_version
   ...
   WHEN 3 THEN pg_catalog.to_regclass('agent_private.platform_grant_revocation_receipts') IS NOT NULL AND pg_catalog.to_regprocedure('agent_private.issue_initial_enrollment_grant(uuid,uuid,uuid,uuid,timestamptz,timestamptz,bytea,bytea)') IS NOT NULL AND pg_catalog.to_regprocedure('agent_private.read_initial_enrollment_grant(uuid,uuid,uuid,uuid,uuid,timestamptz,bytea,bytea,timestamptz,timestamptz,smallint,timestamptz)') IS NOT NULL AND pg_catalog.to_regprocedure('agent_private.revoke_initial_enrollment_grant(uuid,uuid,uuid,uuid,uuid,uuid,timestamptz,bytea,bytea,timestamptz,timestamptz,smallint,timestamptz,bytea)') IS NOT NULL AND
    ...
    (SELECT pg_catalog.count(*)=1 FROM pg_catalog.pg_constraint WHERE conrelid=pg_catalog.to_regclass('agent_private.platform_grant_receipts') AND conname='platform_grant_receipts_issue_contract' AND pg_catalog.md5(pg_catalog.pg_get_constraintdef(oid))='8d85f8a1bd060af5beef075b75bda13b') AND
    (SELECT pg_catalog.count(*)=1 FROM pg_catalog.pg_constraint WHERE conrelid=pg_catalog.to_regclass('agent_private.platform_grant_revocation_receipts') AND conname='platform_grant_revocation_receipts_issue_contract' AND pg_catalog.md5(pg_catalog.pg_get_constraintdef(oid))='76cb108da4b527ec5db5eac500168e68') AND pg_catalog.to_regprocedure('agent_private.read_initial_enrollment_grant_status(uuid,uuid,uuid,uuid,uuid,timestamptz,bytea,bytea,timestamptz,timestamptz)') IS NULL AND pg_catalog.to_regprocedure('agent_private.read_initial_enrollment_grant_status(uuid,uuid,uuid,uuid,uuid,timestamptz,bytea,bytea,timestamptz,timestamptz,smallint,timestamptz)') IS NULL
   WHEN 4 THEN pg_catalog.to_regclass('agent_private.platform_grant_revocation_receipts') IS NOT NULL AND pg_catalog.to_regprocedure('agent_private.read_initial_enrollment_grant_status(uuid,uuid,uuid,uuid,uuid,timestamptz,bytea,bytea,timestamptz,timestamptz)') IS NOT NULL AND pg_catalog.to_regprocedure('agent_private.read_initial_enrollment_grant_status(uuid,uuid,uuid,uuid,uuid,timestamptz,bytea,bytea,timestamptz,timestamptz,smallint,timestamptz)') IS NOT NULL AND (SELECT pg_catalog.count(*)=8 AND pg_catalog.bool_and(pg_catalog.md5(pg_catalog.btrim(function.prosrc,E' \t\r\n'))=expected.body_hash) FROM (VALUES
      (pg_catalog.to_regprocedure('agent_private.issue_initial_enrollment_grant(uuid,uuid,uuid,uuid,timestamptz,bytea,bytea)'),'1ebfa7beba1c293223ece3f8396fdce6'),
      (pg_catalog.to_regprocedure('agent_private.issue_initial_enrollment_grant(uuid,uuid,uuid,uuid,timestamptz,timestamptz,bytea,bytea)'),'1bf8dc3b09be84738a26248171cc7388'),
      (pg_catalog.to_regprocedure('agent_private.read_initial_enrollment_grant(uuid,uuid,uuid,uuid,uuid,timestamptz,bytea,bytea,timestamptz,timestamptz,smallint,timestamptz)'),'79947e1d1e6ce34ef418c9f12eda2b1f'),
      (pg_catalog.to_regprocedure('agent_private.read_initial_enrollment_grant(uuid,uuid,uuid,uuid,uuid,timestamptz,bytea,bytea,timestamptz,timestamptz)'),'083af3e33f15d59b9353ef99d68cf0ef'),
