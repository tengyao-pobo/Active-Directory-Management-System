...
SELECT 1/pg_catalog.count(*) AS roles_are_distinct FROM (SELECT 1 WHERE
 ...
 :'agent_table_owner_role'<>:'agent_enrollment_target_role' AND
 :'agent_enrollment_target_definer_role'<>:'agent_enrollment_target_role') checked;
SELECT 1/pg_catalog.count(*) AS exact_definer_reservation FROM agent_private.agent_capability_roles
WHERE role_name=:'agent_enrollment_target_definer_role'::name AND capability='EnrollmentTargetRead' AND role_kind='Definer';
SELECT 1/pg_catalog.count(*) AS runtime_role_is_safe FROM (SELECT 1 WHERE
 (SELECT count(*)=1 FROM pg_catalog.pg_roles role WHERE role.rolname=:'agent_enrollment_target_role' AND role.rolcanlogin AND NOT role.rolsuper AND NOT role.rolbypassrls AND NOT role.rolcreatedb AND NOT role.rolcreaterole AND NOT role.rolinherit AND NOT role.rolreplication) AND
 NOT EXISTS(SELECT 1 FROM pg_catalog.pg_auth_members member JOIN pg_catalog.pg_roles role ON member.member=role.oid OR member.roleid=role.oid WHERE role.rolname=:'agent_enrollment_target_role') AND
 NOT EXISTS(SELECT 1 FROM pg_catalog.pg_database database JOIN pg_catalog.pg_roles role ON role.oid=database.datdba WHERE role.rolname=:'agent_enrollment_target_role') AND
 NOT EXISTS(SELECT 1 FROM pg_catalog.pg_namespace namespace JOIN pg_catalog.pg_roles role ON role.oid=namespace.nspowner WHERE role.rolname=:'agent_enrollment_target_role') AND
 NOT EXISTS(SELECT 1 FROM pg_catalog.pg_class object JOIN pg_catalog.pg_roles role ON role.oid=object.relowner WHERE role.rolname=:'agent_enrollment_target_role') AND
 NOT EXISTS(SELECT 1 FROM pg_catalog.pg_proc function JOIN pg_catalog.pg_roles role ON role.oid=function.proowner WHERE role.rolname=:'agent_enrollment_target_role') AND
 NOT EXISTS(SELECT 1 FROM agent_private.agent_capability_roles reservation WHERE reservation.role_name=:'agent_enrollment_target_role'::name AND (reservation.capability<>'EnrollmentTargetRead' OR reservation.role_kind<>'Runtime')) AND
 NOT EXISTS(SELECT 1 FROM agent_private.enrollment_target_read_database_bindings binding WHERE binding.login_role=:'agent_enrollment_target_role'::name AND (binding.environment_id<>:'environment_id'::uuid OR binding.purpose<>'ResolveEnrollmentTarget')) AND
 NOT pg_catalog.has_schema_privilege(:'agent_enrollment_target_role','agent_private','CREATE')) checked;

INSERT INTO agent_private.agent_capability_roles(role_name,capability,role_kind)
VALUES(:'agent_enrollment_target_role'::name,'EnrollmentTargetRead','Runtime')
ON CONFLICT(role_name) DO NOTHING;
SELECT 1/pg_catalog.count(*) AS exact_runtime_reservation FROM agent_private.agent_capability_roles
WHERE role_name=:'agent_enrollment_target_role'::name AND capability='EnrollmentTargetRead' AND role_kind='Runtime';
ALTER ROLE :"agent_enrollment_target_role" LOGIN NOSUPERUSER NOCREATEDB NOCREATEROLE NOINHERIT NOBYPASSRLS NOREPLICATION;
GRANT CONNECT ON DATABASE :DBNAME TO :"agent_enrollment_target_role";
GRANT USAGE ON SCHEMA agent_private TO :"agent_enrollment_target_role";
REVOKE ALL ON ALL TABLES IN SCHEMA agent_private FROM :"agent_enrollment_target_role";
REVOKE ALL ON ALL SEQUENCES IN SCHEMA agent_private FROM :"agent_enrollment_target_role";
REVOKE ALL ON ALL FUNCTIONS IN SCHEMA agent_private FROM :"agent_enrollment_target_role";
GRANT EXECUTE ON FUNCTION agent_private.resolve_enrollment_target(uuid,uuid),agent_private.audit_enrollment_target_read_privileges(uuid,name,name) TO :"agent_enrollment_target_role";
INSERT INTO agent_private.enrollment_target_read_database_bindings(login_role,environment_id,purpose)
VALUES(:'agent_enrollment_target_role'::name,:'environment_id'::uuid,'ResolveEnrollmentTarget') ON CONFLICT DO NOTHING;
SELECT 1/pg_catalog.count(*) AS exact_binding FROM agent_private.enrollment_target_read_database_bindings
WHERE login_role=:'agent_enrollment_target_role'::name AND environment_id=:'environment_id'::uuid AND purpose='ResolveEnrollmentTarget';


WITH login AS(SELECT role.* FROM pg_catalog.pg_roles role WHERE role.rolname=:'agent_enrollment_target_role'),
 function_owner AS(SELECT role.* FROM pg_catalog.pg_roles role WHERE role.rolname=:'agent_enrollment_target_definer_role'::name),
 ...
 checks AS(SELECT
  ...
  agent_private.agent_role_has_capability(:'agent_enrollment_target_role'::name,'EnrollmentTargetRead','Runtime') AND
  agent_private.agent_role_has_capability(:'agent_enrollment_target_definer_role'::name,'EnrollmentTargetRead','Definer') AND
  ...
  (SELECT count(*)=1 FROM agent_private.enrollment_target_read_database_bindings binding WHERE binding.login_role=:'agent_enrollment_target_role'::name AND binding.environment_id=:'environment_id'::uuid AND binding.purpose='ResolveEnrollmentTarget') AND
  ...
  NOT pg_catalog.has_schema_privilege(:'agent_enrollment_target_role','agent_private','CREATE') AND NOT pg_catalog.has_schema_privilege(:'agent_enrollment_target_definer_role'::name,'agent_private','CREATE') AND
  NOT EXISTS(SELECT 1 FROM pg_catalog.pg_class object,schema_info WHERE object.relnamespace=schema_info.oid AND object.relkind IN('r','p','v','m') AND (pg_catalog.has_table_privilege(:'agent_enrollment_target_role',object.oid,'SELECT') OR pg_catalog.has_table_privilege(:'agent_enrollment_target_role',object.oid,'INSERT') OR pg_catalog.has_table_privilege(:'agent_enrollment_target_role',object.oid,'UPDATE') OR pg_catalog.has_table_privilege(:'agent_enrollment_target_role',object.oid,'DELETE') OR pg_catalog.has_table_privilege(:'agent_enrollment_target_role',object.oid,'TRUNCATE') OR pg_catalog.has_table_privilege(:'agent_enrollment_target_role',object.oid,'REFERENCES') OR pg_catalog.has_table_privilege(:'agent_enrollment_target_role',object.oid,'TRIGGER'))) AND
  ...
  NOT EXISTS(SELECT 1 FROM pg_catalog.pg_proc function,schema_info WHERE function.pronamespace=schema_info.oid AND pg_catalog.has_function_privilege(:'agent_enrollment_target_role',function.oid,'EXECUTE') AND function.oid<>ALL(ARRAY['agent_private.resolve_enrollment_target(uuid,uuid)'::pg_catalog.regprocedure,'agent_private.audit_enrollment_target_read_privileges(uuid,name,name)'::pg_catalog.regprocedure]::oid[])) AND
