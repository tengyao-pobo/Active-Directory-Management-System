...
BEGIN
 IF p_environment_id IS NULL OR p_environment_id='00000000-0000-0000-0000-000000000000'::uuid OR
    ...
    NOT EXISTS(SELECT 1 FROM agent_private.agent_projection_database_bindings binding WHERE
      binding.login_role=SESSION_USER::name AND binding.environment_id=p_environment_id AND binding.purpose='ReadDeviceProjection') THEN
...
CREATE OR REPLACE FUNCTION agent_private.read_current_inventory_projection(p_environment_id uuid,p_directory_object_id uuid)
RETURNS TABLE(outcome text,diagnostic_code text,environment_id uuid,directory_object_id uuid,device_id uuid,
 registration_id uuid,registration_epoch bigint,sequence bigint,receipt_id uuid,collected_at timestamptz,
 received_at timestamptz,last_seen_at timestamptz,basic_json text,software_json text,hardware_json text)
LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,agent_private,pg_temp AS $function$
#variable_conflict use_variable
DECLARE
 v_device_id uuid;v_device_state text;v_last_seen timestamptz;v_registration_id uuid;v_registration_epoch bigint;
 v_sequence bigint;v_receipt_id uuid;v_received_at timestamptz;v_collected timestamptz;v_payload jsonb;
 v_name text;v_collector jsonb;v_data jsonb;v_result jsonb;v_count integer;v_status integer;v_quality integer;v_item_count integer;v_observed timestamptz;
 v_basic jsonb:=pg_catalog.jsonb_build_object('Availability','Missing','DiagnosticCode','ProjectionMissing','Source',NULL,'SourceObservedAt',NULL,'IsTruncated',false,'HostName',NULL,'OperatingSystem',NULL,'NetworkInterfaces','[]'::jsonb);
 v_software jsonb:=pg_catalog.jsonb_build_object('Availability','Missing','DiagnosticCode','ProjectionMissing','Source',NULL,'SourceObservedAt',NULL,'IsTruncated',false,'Applications','[]'::jsonb);
 v_hardware jsonb:=pg_catalog.jsonb_build_object('Availability','Missing','DiagnosticCode','ProjectionMissing','Source',NULL,'SourceObservedAt',NULL,'Sections','[]'::jsonb);
 v_rows jsonb;v_sections jsonb;v_section jsonb;v_row jsonb;v_built_row jsonb;v_value jsonb;v_text text;v_source text;v_availability text;v_section_diagnostic text;
 v_kind integer;v_section_quality integer;v_section_observed timestamptz;v_total_rows integer:=0;v_any_observed boolean:=false;v_nonnull boolean;v_props text[];v_numeric text[];v_property text;
BEGIN
 IF p_environment_id IS NULL OR p_environment_id='00000000-0000-0000-0000-000000000000'::uuid OR p_directory_object_id IS NULL OR p_directory_object_id='00000000-0000-0000-0000-000000000000'::uuid OR
    NOT EXISTS(SELECT 1 FROM agent_private.agent_projection_database_bindings binding WHERE binding.login_role=SESSION_USER::name AND binding.environment_id=p_environment_id AND binding.purpose='ReadDeviceProjection') THEN
  RETURN QUERY SELECT 'Unavailable','AuthenticationFailed',p_environment_id,p_directory_object_id,NULL::uuid,NULL::uuid,NULL::bigint,NULL::bigint,NULL::uuid,NULL::timestamptz,NULL::timestamptz,NULL::timestamptz,v_basic::text,v_software::text,v_hardware::text;RETURN;
 END IF;
 SELECT device.device_id,device.state,device.last_seen_at INTO v_device_id,v_device_state,v_last_seen FROM agent_private.agent_device_directory_bindings mapping JOIN agent_private.devices device ON device.environment_id=mapping.environment_id AND device.device_id=mapping.device_id WHERE mapping.environment_id=p_environment_id AND mapping.directory_object_id=p_directory_object_id;
 IF NOT FOUND THEN RETURN QUERY SELECT 'Missing','MappingMissing',p_environment_id,p_directory_object_id,NULL::uuid,NULL::uuid,NULL::bigint,NULL::bigint,NULL::uuid,NULL::timestamptz,NULL::timestamptz,NULL::timestamptz,v_basic::text,v_software::text,v_hardware::text;RETURN;END IF;
 IF v_device_state<>'Active' THEN RETURN QUERY SELECT 'Missing','DeviceUnavailable',p_environment_id,p_directory_object_id,v_device_id,NULL::uuid,NULL::bigint,NULL::bigint,NULL::uuid,NULL::timestamptz,NULL::timestamptz,v_last_seen,v_basic::text,v_software::text,v_hardware::text;RETURN;END IF;
 SELECT registration.registration_id,registration.registration_epoch INTO STRICT v_registration_id,v_registration_epoch FROM agent_private.registrations registration WHERE registration.environment_id=p_environment_id AND registration.device_id=v_device_id AND registration.state='Active';
 BEGIN SELECT projection.sequence,projection.receipt_id,projection.normalized_payload INTO STRICT v_sequence,v_receipt_id,v_payload FROM agent_private.inventory_projection projection WHERE projection.environment_id=p_environment_id AND projection.device_id=v_device_id AND projection.registration_id=v_registration_id AND projection.registration_epoch=v_registration_epoch;
 EXCEPTION WHEN NO_DATA_FOUND THEN RETURN QUERY SELECT 'Missing','ProjectionMissing',p_environment_id,p_directory_object_id,v_device_id,v_registration_id,v_registration_epoch,NULL::bigint,NULL::uuid,NULL::timestamptz,NULL::timestamptz,v_last_seen,v_basic::text,v_software::text,v_hardware::text;RETURN;END;
 BEGIN SELECT receipt.received_at INTO STRICT v_received_at FROM agent_private.receipts receipt WHERE receipt.environment_id=p_environment_id AND receipt.device_id=v_device_id AND receipt.registration_id=v_registration_id AND receipt.registration_epoch=v_registration_epoch AND receipt.sequence=v_sequence AND receipt.receipt_id=v_receipt_id;
 EXCEPTION WHEN NO_DATA_FOUND THEN RAISE EXCEPTION 'missing receipt';END;
 IF pg_catalog.octet_length(v_payload::text)>524288 OR pg_catalog.jsonb_typeof(v_payload)<>'object' OR NOT(v_payload ?& ARRAY['SchemaVersion','CollectedAt','Collectors']) OR v_payload-ARRAY['SchemaVersion','CollectedAt','Collectors']<>'{}'::jsonb OR v_payload->'SchemaVersion'<>'1'::jsonb OR pg_catalog.jsonb_typeof(v_payload->'CollectedAt')<>'string' OR NOT((v_payload->>'CollectedAt')~'^[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}([.][0-9]{1,7})?(Z|[+-][0-9]{2}:[0-9]{2})$') OR pg_catalog.jsonb_typeof(v_payload->'Collectors')<>'array' OR pg_catalog.jsonb_array_length(v_payload->'Collectors')>32 THEN RAISE EXCEPTION 'invalid projection';END IF;
 BEGIN v_collected:=(v_payload->>'CollectedAt')::timestamptz;EXCEPTION WHEN OTHERS THEN RAISE EXCEPTION 'invalid projection';END;
 IF NOT pg_catalog.isfinite(v_collected) OR v_collected>pg_catalog.clock_timestamp()+interval '5 minutes' OR NOT pg_catalog.isfinite(v_received_at) OR v_received_at>pg_catalog.clock_timestamp()+interval '5 minutes' THEN RAISE EXCEPTION 'invalid projection';END IF;

 FOREACH v_name IN ARRAY ARRAY['basic-device','installed-software','hardware'] LOOP
  SELECT pg_catalog.count(*),(pg_catalog.jsonb_agg(value))->0 INTO v_count,v_collector FROM pg_catalog.jsonb_array_elements(v_payload->'Collectors') item(value) WHERE value->>'Collector'=v_name;
  IF v_count=0 THEN CONTINUE;END IF;
  BEGIN
   IF v_count<>1 OR pg_catalog.jsonb_typeof(v_collector)<>'object' OR NOT(v_collector ?& ARRAY['Collector','Status','Quality','Source','ObservedAt','Data','ItemCount','ErrorCode']) OR v_collector-ARRAY['Collector','Status','Quality','Source','ObservedAt','Data','ItemCount','ErrorCode']<>'{}'::jsonb OR
      pg_catalog.jsonb_typeof(v_collector->'Status')<>'number' OR NOT((v_collector->>'Status')~'^[0-3]$') OR pg_catalog.jsonb_typeof(v_collector->'Quality')<>'number' OR NOT((v_collector->>'Quality')~'^[0-3]$') OR pg_catalog.jsonb_typeof(v_collector->'ItemCount')<>'number' OR NOT((v_collector->>'ItemCount')~'^[0-9]{1,5}$') OR (v_collector->>'ItemCount')::numeric>10000 OR pg_catalog.jsonb_typeof(v_collector->'ObservedAt')<>'string' OR NOT((v_collector->>'ObservedAt')~'^[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}([.][0-9]{1,7})?(Z|[+-][0-9]{2}:[0-9]{2})$') THEN RAISE EXCEPTION 'invalid collector';END IF;
   v_status:=(v_collector->>'Status')::integer;v_quality:=(v_collector->>'Quality')::integer;v_item_count:=(v_collector->>'ItemCount')::integer;
   BEGIN v_observed:=(v_collector->>'ObservedAt')::timestamptz;EXCEPTION WHEN OTHERS THEN RAISE EXCEPTION 'invalid collector';END;
   IF NOT pg_catalog.isfinite(v_observed) OR v_observed>pg_catalog.clock_timestamp()+interval '5 minutes' THEN RAISE EXCEPTION 'invalid collector';END IF;
   IF v_status<>0 THEN
    IF v_quality<>1 OR v_collector->'Data'<>'null'::jsonb OR v_item_count<>0 OR (v_collector->>'Source') IS DISTINCT FROM v_name OR
       NOT((v_status=1 AND v_collector->>'ErrorCode'='timeout') OR (v_status=2 AND v_collector->>'ErrorCode'='collector_failed') OR (v_status=3 AND v_collector->>'ErrorCode'='output_limit_exceeded')) THEN RAISE EXCEPTION 'invalid failed collector';END IF;
    v_result:=pg_catalog.jsonb_build_object('Availability','Unavailable','DiagnosticCode','SourceUnavailable','Source',NULL,'SourceObservedAt',NULL,'IsTruncated',false);
    IF v_name='basic-device' THEN v_result:=v_result||pg_catalog.jsonb_build_object('HostName',NULL,'OperatingSystem',NULL,'NetworkInterfaces','[]'::jsonb);
    ELSIF v_name='installed-software' THEN v_result:=v_result||pg_catalog.jsonb_build_object('Applications','[]'::jsonb);
    ELSE v_result:=v_result-'IsTruncated'||pg_catalog.jsonb_build_object('Sections','[]'::jsonb);END IF;
   ELSIF v_name='basic-device' THEN
    v_data:=v_collector->'Data';
    IF v_quality<>0 OR (v_collector->>'Source') IS DISTINCT FROM 'basic-device' OR v_collector->'ErrorCode'<>'null'::jsonb OR pg_catalog.jsonb_typeof(v_data)<>'object' OR NOT(v_data ?& ARRAY['HostName','OperatingSystem','NetworkInterfaces','IsTruncated']) OR (v_data-ARRAY['HostName','OperatingSystem','NetworkInterfaces','IsTruncated'])<>'{}'::jsonb THEN RAISE EXCEPTION 'invalid basic';END IF;
    IF pg_catalog.jsonb_typeof(v_data->'HostName')<>'string' OR pg_catalog.length(v_data->>'HostName')>512 OR (v_data->>'HostName')~'[[:cntrl:]]' OR pg_catalog.jsonb_typeof(v_data->'OperatingSystem')<>'object' OR NOT((v_data->'OperatingSystem') ?& ARRAY['Description','Version','Architecture']) OR ((v_data->'OperatingSystem')-ARRAY['Description','Version','Architecture'])<>'{}'::jsonb THEN RAISE EXCEPTION 'invalid basic';END IF;
    IF EXISTS(SELECT 1 FROM pg_catalog.jsonb_each(v_data->'OperatingSystem') property WHERE pg_catalog.jsonb_typeof(property.value)<>'string' OR pg_catalog.length(property.value #>> '{}')>512 OR (property.value #>> '{}')~'[[:cntrl:]]') OR pg_catalog.jsonb_typeof(v_data->'NetworkInterfaces')<>'array' OR pg_catalog.jsonb_array_length(v_data->'NetworkInterfaces')>64 OR pg_catalog.jsonb_typeof(v_data->'IsTruncated')<>'boolean' OR v_item_count<>1+pg_catalog.jsonb_array_length(v_data->'NetworkInterfaces') THEN RAISE EXCEPTION 'invalid basic';END IF;
    IF EXISTS(SELECT 1 FROM pg_catalog.jsonb_array_elements(v_data->'NetworkInterfaces') interface WHERE pg_catalog.jsonb_typeof(interface)<>'object' OR NOT(interface ?& ARRAY['Name','InterfaceType','Addresses','Gateways','DnsServers','MacAddress']) OR interface-ARRAY['Name','InterfaceType','Addresses','Gateways','DnsServers','MacAddress']<>'{}'::jsonb OR
      pg_catalog.jsonb_typeof(interface->'Name')<>'string' OR pg_catalog.length(interface->>'Name')>512 OR interface->>'Name'~'[[:cntrl:]]' OR pg_catalog.jsonb_typeof(interface->'InterfaceType')<>'string' OR pg_catalog.length(interface->>'InterfaceType')>512 OR interface->>'InterfaceType'~'[[:cntrl:]]' OR
      NOT(interface->'MacAddress'='null'::jsonb OR (pg_catalog.jsonb_typeof(interface->'MacAddress')='string' AND pg_catalog.length(interface->>'MacAddress')<=512 AND NOT(interface->>'MacAddress'~'[[:cntrl:]]'))) OR
      EXISTS(SELECT 1 FROM pg_catalog.jsonb_each(interface) property WHERE property.key IN('Addresses','Gateways','DnsServers') AND (pg_catalog.jsonb_typeof(property.value)<>'array' OR pg_catalog.jsonb_array_length(property.value)>32 OR EXISTS(SELECT 1 FROM pg_catalog.jsonb_array_elements(property.value) element WHERE pg_catalog.jsonb_typeof(element)<>'string' OR pg_catalog.length(element#>>'{}')>512 OR element#>>'{}'~'[[:cntrl:]]')))) THEN RAISE EXCEPTION 'invalid basic';END IF;
    SELECT COALESCE(pg_catalog.jsonb_agg(pg_catalog.jsonb_build_object('Name',interface->'Name','InterfaceType',interface->'InterfaceType','Addresses',interface->'Addresses','Gateways',interface->'Gateways','DnsServers',interface->'DnsServers','MacAddress',interface->'MacAddress') ORDER BY ordinal),'[]'::jsonb) INTO v_rows FROM pg_catalog.jsonb_array_elements(v_data->'NetworkInterfaces') WITH ORDINALITY item(interface,ordinal);
    v_result:=pg_catalog.jsonb_build_object('Availability','Observed','DiagnosticCode','None','Source','basic-device','SourceObservedAt',v_observed,'IsTruncated',(v_data->>'IsTruncated')::boolean,'HostName',v_data->'HostName','OperatingSystem',pg_catalog.jsonb_build_object('Description',v_data->'OperatingSystem'->'Description','Version',v_data->'OperatingSystem'->'Version','Architecture',v_data->'OperatingSystem'->'Architecture'),'NetworkInterfaces',v_rows);
   ELSIF v_name='installed-software' THEN
    v_data:=v_collector->'Data';IF v_quality NOT IN(0,3) OR (v_collector->>'Source') IS DISTINCT FROM 'HKLM uninstall registry (32-bit and 64-bit views)' OR v_collector->'ErrorCode'<>'null'::jsonb OR pg_catalog.jsonb_typeof(v_data)<>'object' OR NOT(v_data ?& ARRAY['Applications','IsTruncated']) OR v_data-ARRAY['Applications','IsTruncated']<>'{}'::jsonb OR pg_catalog.jsonb_typeof(v_data->'Applications')<>'array' OR pg_catalog.jsonb_array_length(v_data->'Applications')>10000 OR pg_catalog.jsonb_typeof(v_data->'IsTruncated')<>'boolean' OR v_item_count<>pg_catalog.jsonb_array_length(v_data->'Applications') OR (v_quality=3 AND (pg_catalog.jsonb_array_length(v_data->'Applications')<>0 OR (v_data->>'IsTruncated')::boolean)) THEN RAISE EXCEPTION 'invalid software';END IF;
    IF v_quality=3 THEN v_result:=pg_catalog.jsonb_build_object('Availability','Unavailable','DiagnosticCode','SourceUnavailable','Source',NULL,'SourceObservedAt',NULL,'IsTruncated',false,'Applications','[]'::jsonb);
    ELSE
     IF EXISTS(SELECT 1 FROM pg_catalog.jsonb_array_elements(v_data->'Applications') application WHERE pg_catalog.jsonb_typeof(application)<>'object' OR NOT(application ?& ARRAY['Name','Version','Publisher','InstallDate','Architecture']) OR application-ARRAY['Name','Version','Publisher','InstallDate','Architecture']<>'{}'::jsonb OR pg_catalog.jsonb_typeof(application->'Name')<>'string' OR pg_catalog.btrim(application->>'Name')='' OR pg_catalog.length(application->>'Name')>512 OR application->>'Name'~'[[:cntrl:]]' OR application->>'Architecture' NOT IN('x86','x64') OR EXISTS(SELECT 1 FROM pg_catalog.jsonb_each(application) property WHERE property.key IN('Version','Publisher','InstallDate') AND NOT(property.value='null'::jsonb OR (pg_catalog.jsonb_typeof(property.value)='string' AND pg_catalog.length(property.value#>>'{}')<=512 AND NOT(property.value#>>'{}'~'[[:cntrl:]]'))))) THEN RAISE EXCEPTION 'invalid software';END IF;
     SELECT COALESCE(pg_catalog.jsonb_agg(pg_catalog.jsonb_build_object('Name',application->'Name','Version',application->'Version','Publisher',application->'Publisher','InstallDate',application->'InstallDate','Architecture',application->'Architecture') ORDER BY ordinal),'[]'::jsonb) INTO v_rows FROM pg_catalog.jsonb_array_elements(v_data->'Applications') WITH ORDINALITY item(application,ordinal);
     v_result:=pg_catalog.jsonb_build_object('Availability','Observed','DiagnosticCode','None','Source','HKLM uninstall registry (32-bit and 64-bit views)','SourceObservedAt',v_observed,'IsTruncated',(v_data->>'IsTruncated')::boolean,'Applications',v_rows);
    END IF;
   ELSE
    v_data:=v_collector->'Data';IF v_quality NOT IN(0,1) OR (v_collector->>'Source') IS DISTINCT FROM 'hardware' OR v_collector->'ErrorCode'<>'null'::jsonb OR pg_catalog.jsonb_typeof(v_data)<>'object' OR NOT(v_data ?& ARRAY['SchemaVersion','Sections']) OR v_data-ARRAY['SchemaVersion','Sections']<>'{}'::jsonb OR v_data->'SchemaVersion'<>'1'::jsonb OR pg_catalog.jsonb_typeof(v_data->'Sections')<>'array' OR pg_catalog.jsonb_array_length(v_data->'Sections')<>9 OR (SELECT pg_catalog.count(DISTINCT section->>'Kind') FROM pg_catalog.jsonb_array_elements(v_data->'Sections') section)<>9 THEN RAISE EXCEPTION 'invalid hardware';END IF;
    v_sections:='[]'::jsonb;v_total_rows:=0;v_any_observed:=false;
    FOR v_section IN SELECT value FROM pg_catalog.jsonb_array_elements(v_data->'Sections') WITH ORDINALITY item(value,ordinal) ORDER BY ordinal LOOP
     IF pg_catalog.jsonb_typeof(v_section)<>'object' OR NOT(v_section ?& ARRAY['Kind','Source','Quality','ObservedAt','Rows','IsTruncated','ErrorCode']) OR v_section-ARRAY['Kind','Source','Quality','ObservedAt','Rows','IsTruncated','ErrorCode']<>'{}'::jsonb OR pg_catalog.jsonb_typeof(v_section->'Kind')<>'number' OR NOT((v_section->>'Kind')~'^[0-8]$') OR pg_catalog.jsonb_typeof(v_section->'Quality')<>'number' OR NOT((v_section->>'Quality')~'^[0-3]$') OR pg_catalog.jsonb_typeof(v_section->'ObservedAt')<>'string' OR NOT((v_section->>'ObservedAt')~'^[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}([.][0-9]{1,7})?(Z|[+-][0-9]{2}:[0-9]{2})$') OR pg_catalog.jsonb_typeof(v_section->'Rows')<>'array' OR pg_catalog.jsonb_array_length(v_section->'Rows')>128 OR pg_catalog.jsonb_typeof(v_section->'IsTruncated')<>'boolean' THEN RAISE EXCEPTION 'invalid section';END IF;
     v_kind:=(v_section->>'Kind')::integer;v_section_quality:=(v_section->>'Quality')::integer;BEGIN v_section_observed:=(v_section->>'ObservedAt')::timestamptz;EXCEPTION WHEN OTHERS THEN RAISE EXCEPTION 'invalid section';END;
     v_source:=CASE v_kind WHEN 0 THEN 'Win32_ComputerSystem' WHEN 1 THEN 'Win32_ComputerSystemProduct' WHEN 2 THEN 'Win32_BIOS' WHEN 3 THEN 'Win32_OperatingSystem' WHEN 4 THEN 'Win32_Processor' WHEN 5 THEN 'Win32_PhysicalMemory' WHEN 6 THEN 'Win32_VideoController' WHEN 7 THEN 'Win32_DiskDrive' WHEN 8 THEN 'Win32_Battery' END;
     IF NOT pg_catalog.isfinite(v_section_observed) OR v_section_observed>pg_catalog.clock_timestamp()+interval '5 minutes' OR (v_section->>'Source') IS DISTINCT FROM v_source OR NOT((v_section_quality=0 AND pg_catalog.jsonb_array_length(v_section->'Rows')>0 AND v_section->'ErrorCode'='null'::jsonb) OR (v_section_quality=2 AND v_kind IN(6,8) AND pg_catalog.jsonb_array_length(v_section->'Rows')=0 AND v_section->'ErrorCode'='null'::jsonb AND NOT (v_section->>'IsTruncated')::boolean) OR (v_section_quality=3 AND pg_catalog.jsonb_array_length(v_section->'Rows')=0 AND v_section->>'ErrorCode'='access_denied' AND NOT (v_section->>'IsTruncated')::boolean) OR (v_section_quality=1 AND pg_catalog.jsonb_array_length(v_section->'Rows')=0 AND (v_section->'ErrorCode'='null'::jsonb OR v_section->>'ErrorCode' IN('query_timeout','query_unavailable')))) THEN RAISE EXCEPTION 'invalid section';END IF;
     v_props:=CASE v_kind WHEN 0 THEN ARRAY['Manufacturer','Model','TotalPhysicalMemory'] WHEN 1 THEN ARRAY['UUID'] WHEN 2 THEN ARRAY['Manufacturer','SerialNumber','SMBIOSBIOSVersion','ReleaseDate'] WHEN 3 THEN ARRAY['Caption','Version','BuildNumber','InstallDate','LastBootUpTime'] WHEN 4 THEN ARRAY['Name','Manufacturer','NumberOfCores','NumberOfLogicalProcessors'] WHEN 5 THEN ARRAY['BankLabel','DeviceLocator','Capacity','Speed'] WHEN 6 THEN ARRAY['Name','AdapterRAM'] WHEN 7 THEN ARRAY['Model','SerialNumber','Size','MediaType'] WHEN 8 THEN ARRAY['Name','EstimatedChargeRemaining','DesignCapacity','FullChargeCapacity'] END;
     v_numeric:=CASE v_kind WHEN 0 THEN ARRAY['TotalPhysicalMemory'] WHEN 4 THEN ARRAY['NumberOfCores','NumberOfLogicalProcessors'] WHEN 5 THEN ARRAY['Capacity','Speed'] WHEN 6 THEN ARRAY['AdapterRAM'] WHEN 7 THEN ARRAY['Size'] WHEN 8 THEN ARRAY['EstimatedChargeRemaining','DesignCapacity','FullChargeCapacity'] ELSE ARRAY[]::text[] END;
     v_rows:='[]'::jsonb;
     FOR v_row IN SELECT value FROM pg_catalog.jsonb_array_elements(v_section->'Rows') WITH ORDINALITY item(value,ordinal) ORDER BY ordinal LOOP
      IF pg_catalog.jsonb_typeof(v_row)<>'object' OR NOT(v_row ?& v_props) OR v_row-v_props<>'{}'::jsonb THEN RAISE EXCEPTION 'invalid hardware row';END IF;v_built_row:='{}'::jsonb;v_nonnull:=false;
      FOREACH v_property IN ARRAY v_props LOOP v_value:=v_row->v_property;
       IF v_value<>'null'::jsonb THEN v_nonnull:=true;
        IF v_property=ANY(v_numeric) THEN IF pg_catalog.jsonb_typeof(v_value)<>'number' OR NOT((v_value#>>'{}')~'^(0|[1-9][0-9]{0,19})$') OR (v_value#>>'{}')::numeric>18446744073709551615 THEN RAISE EXCEPTION 'invalid hardware number';END IF;v_text:=((v_value#>>'{}')::numeric)::text;v_built_row:=v_built_row||pg_catalog.jsonb_build_object(v_property,v_text);
        ELSE IF pg_catalog.jsonb_typeof(v_value)<>'string' OR pg_catalog.length(v_value#>>'{}')>512 OR v_value#>>'{}'~'[[:cntrl:]]' THEN RAISE EXCEPTION 'invalid hardware text';END IF;v_built_row:=v_built_row||pg_catalog.jsonb_build_object(v_property,v_value);END IF;
       ELSE v_built_row:=v_built_row||pg_catalog.jsonb_build_object(v_property,NULL);END IF;
      END LOOP;
      IF NOT v_nonnull THEN RAISE EXCEPTION 'empty hardware row';END IF;v_rows:=v_rows||pg_catalog.jsonb_build_array(v_built_row);
     END LOOP;
     v_total_rows:=v_total_rows+pg_catalog.jsonb_array_length(v_section->'Rows');v_any_observed:=v_any_observed OR v_section_quality=0;
     v_availability:=CASE v_section_quality WHEN 0 THEN 'Observed' WHEN 2 THEN 'NotApplicable' ELSE 'Unavailable' END;v_section_diagnostic:=CASE WHEN v_section_quality IN(0,2) THEN 'None' ELSE 'SourceUnavailable' END;
     v_sections:=v_sections||pg_catalog.jsonb_build_array(pg_catalog.jsonb_build_object('Kind',v_kind,'Source',v_source,'Availability',v_availability,'DiagnosticCode',v_section_diagnostic,'SourceObservedAt',v_section_observed,'IsTruncated',(v_section->>'IsTruncated')::boolean,'Rows',v_rows));
    END LOOP;
    IF v_item_count<>v_total_rows OR (v_any_observed AND v_quality<>0) OR (NOT v_any_observed AND v_quality<>1) THEN RAISE EXCEPTION 'invalid hardware summary';END IF;
    v_result:=pg_catalog.jsonb_build_object('Availability','Observed','DiagnosticCode','None','Source','hardware','SourceObservedAt',v_observed,'Sections',v_sections);
   END IF;
  EXCEPTION WHEN OTHERS THEN
   v_result:=pg_catalog.jsonb_build_object('Availability','Unavailable','DiagnosticCode','ProjectionMalformed','Source',NULL,'SourceObservedAt',NULL,'IsTruncated',false);
   IF v_name='basic-device' THEN v_result:=v_result||pg_catalog.jsonb_build_object('HostName',NULL,'OperatingSystem',NULL,'NetworkInterfaces','[]'::jsonb);ELSIF v_name='installed-software' THEN v_result:=v_result||pg_catalog.jsonb_build_object('Applications','[]'::jsonb);ELSE v_result:=v_result-'IsTruncated'||pg_catalog.jsonb_build_object('Sections','[]'::jsonb);END IF;
   IF v_name='basic-device' THEN v_result:=v_result||pg_catalog.jsonb_build_object('HostName',v_text||':'||SQLERRM);END IF;
  END;
  IF v_name='basic-device' THEN v_basic:=v_result;ELSIF v_name='installed-software' THEN v_software:=v_result;ELSE v_hardware:=v_result;END IF;
 END LOOP;
 RETURN QUERY SELECT 'Observed','None',p_environment_id,p_directory_object_id,v_device_id,v_registration_id,v_registration_epoch,v_sequence,v_receipt_id,v_collected,v_received_at,v_last_seen,v_basic::text,v_software::text,v_hardware::text;
EXCEPTION
 WHEN NO_DATA_FOUND THEN RETURN QUERY SELECT 'Missing','RegistrationUnavailable',p_environment_id,p_directory_object_id,v_device_id,NULL::uuid,NULL::bigint,NULL::bigint,NULL::uuid,NULL::timestamptz,NULL::timestamptz,v_last_seen,v_basic::text,v_software::text,v_hardware::text;
 WHEN OTHERS THEN RETURN QUERY SELECT 'Unavailable','ProjectionMalformed',p_environment_id,p_directory_object_id,v_device_id,v_registration_id,v_registration_epoch,v_sequence,v_receipt_id,NULL::timestamptz,v_received_at,v_last_seen,v_basic::text,v_software::text,v_hardware::text;
END;
$function$;
